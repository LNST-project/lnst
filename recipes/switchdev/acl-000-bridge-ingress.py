...
def do_task(ctl, hosts, ifaces, aliases):
    ...
    sw.run("tc filter add dev %s parent ffff: protocol ipv6 pref 11 flower skip_sw dst_ip %s action drop" % (sw_if1.get_devname(),test_ip_nomask(1,2)[1]))
    assert_procs = run_packet_assert(0, sw_if2, m1_if1, m2_if1,
                                     aliases["ipv"])
    tl.ping_simple(m1_if1, m2_if1, fail_expected=True, count=10)
    for assert_proc in assert_procs:
        assert_proc.intr()
    sw.run("tc filter del dev %s parent ffff: protocol ip pref 10 flower skip_sw dst_ip %s action drop" % (sw_if1.get_devname(),test_ip_nomask(1,2)[0]))
    sw.run("tc filter del dev %s parent ffff: protocol ipv6 pref 11 flower skip_sw dst_ip %s action drop" % (sw_if1.get_devname(),test_ip_nomask(1,2)[1]))

    # src_ip
    tl.ping_simple(m1_if1, m2_if1, count=10)
    sw.run("tc filter add dev %s parent ffff: protocol ip pref 10 flower skip_sw src_ip %s action drop" % (sw_if1.get_devname(),test_ip_nomask(1,1)[0]))
    sw.run("tc filter add dev %s parent ffff: protocol ipv6 pref 11 flower skip_sw src_ip %s action drop" % (sw_if1.get_devname(),test_ip_nomask(1,1)[1]))
    assert_procs = run_packet_assert(0, sw_if2, m1_if1, m2_if1,
                                     aliases["ipv"])
    tl.ping_simple(m1_if1, m2_if1, fail_expected=True, count=10)
    for assert_proc in assert_procs:
        assert_proc.intr()
    sw.run("tc filter del dev %s parent ffff: protocol ip pref 10 flower skip_sw src_ip %s action drop" % (sw_if1.get_devname(),test_ip_nomask(1,1)[0]))
    sw.run("tc filter del dev %s parent ffff: protocol ipv6 pref 11 flower skip_sw src_ip %s action drop" % (sw_if1.get_devname(),test_ip_nomask(1,1)[1]))

    # dst_mac
    tl.ping_simple(m1_if1, m2_if1, count=10)
    sw.run("tc filter add dev %s parent ffff: protocol ip pref 10 flower skip_sw dst_mac %s action drop" % (sw_if1.get_devname(),m2_if1.get_hwaddr()))
    sw.run("tc filter add dev %s parent ffff: protocol ipv6 pref 11 flower skip_sw dst_mac %s action drop" % (sw_if1.get_devname(),m2_if1.get_hwaddr()))
    assert_procs = run_packet_assert(0, sw_if2, m1_if1, m2_if1,
                                     aliases["ipv"])
    tl.ping_simple(m1_if1, m2_if1, fail_expected=True, count=10)
    for assert_proc in assert_procs:
    ...
    sw.run("tc filter add dev %s parent ffff: protocol ipv6 pref 11 flower skip_sw src_mac %s action drop" % (sw_if1.get_devname(),m1_if1.get_hwaddr()))
    assert_procs = run_packet_assert(0, sw_if2, m1_if1, m2_if1,
                                     aliases["ipv"])
    tl.ping_simple(m1_if1, m2_if1, fail_expected=True, count=10)
    for assert_proc in assert_procs:
    ...
    for assert_proc in assert_procs:
        assert_proc.intr()
    sw.run("tc filter del dev %s parent ffff: protocol ip pref 10 flower skip_sw ip_proto tcp dst_port 80 action drop" % sw_if1.get_devname())
   
    # tcp src_port
    m1.run("nping --tcp -p 80 -g 50000 %s" % test_ip_nomask(1,2)[0])
    sw.run("tc filter add dev %s parent ffff: protocol ip pref 10 flower skip_sw ip_proto tcp src_port 50000 action drop" % sw_if1.get_devname())
    assert_procs = run_packet_assert(0, sw_if2, m1_if1, m2_if1,
                                     aliases["ipv"], "tcp", 50000, 80)
    m1.run("nping --tcp -p 80 -g 50000 %s" % test_ip_nomask(1,2)[0])
    for assert_proc in assert_procs:
        assert_proc.intr()
    sw.run("tc filter del dev %s parent ffff: protocol ip pref 10 flower skip_sw ip_proto tcp src_port 50000 action drop" % sw_if1.get_devname())

    # udp dst_port
    m1.run("nping --udp -p 80 -g 50000 %s" % test_ip_nomask(1,2)[0])
    sw.run("tc filter add dev %s parent ffff: protocol ip pref 10 flower skip_sw ip_proto udp dst_port 80 action drop" % sw_if1.get_devname())
    assert_procs = run_packet_assert(0, sw_if2, m1_if1, m2_if1,
                                     aliases["ipv"], "udp", 50000, 80)
    m1.run("nping --udp -p 80 -g 50000 %s" % test_ip_nomask(1,2)[0])
    for assert_proc in assert_procs:
        assert_proc.intr()
    sw.run("tc filter del dev %s parent ffff: protocol ip pref 10 flower skip_sw ip_proto udp dst_port 80 action drop" % sw_if1.get_devname())
   
    # udp src_port
    m1.run("nping --udp -p 80 -g 50000 %s" % test_ip_nomask(1,2)[0])
    sw.run("tc filter add dev %s parent ffff: protocol ip pref 10 flower skip_sw ip_proto udp src_port 50000 action drop" % sw_if1.get_devname())
    assert_procs = run_packet_assert(0, sw_if2, m1_if1, m2_if1,
                                     aliases["ipv"], "udp", 50000, 80)
    m1.run("nping --udp -p 80 -g 50000 %s" % test_ip_nomask(1,2)[0])
    for assert_proc in assert_procs:
        assert_proc.intr()
    sw.run("tc filter del dev %s parent ffff: protocol ip pref 10 flower skip_sw ip_proto udp src_port 50000 action drop" % sw_if1.get_devname())
