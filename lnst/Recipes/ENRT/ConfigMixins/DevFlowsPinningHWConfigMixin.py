...
class DevFlowsPinningHWConfigMixin(BaseHWConfigMixin):
    """
        ...
        :attr:`lnst.RecipeCommon.Perf.Measurements.BaseFlowMeasurement.Flow.receiver_cpupin`
        CPUs, which is required for optimal performance.
    """
    ...
    def hw_config(self, config: EnrtConfiguration):
        ...
        for combination in self.generate_flow_combinations(config):
            for flow in combination:
                ...
                for steering_nic in self.steer_flow_at(flow):
                    if steering_nic not in config.hw_config["flow_steering_rules"]:
                        config.hw_config["flow_steering_rules"][steering_nic] = []
    ...
    def steer_flow_at(self, flow) -> Iterable[Device]:
        """
