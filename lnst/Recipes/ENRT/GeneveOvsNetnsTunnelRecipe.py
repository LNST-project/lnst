...
class GeneveOvsNetnsTunnelRecipe(CommonHWSubConfigMixin, OffloadSubConfigMixin, SimpleNetworkReq, BaseTunnelRecipe):
    ...
    def create_tunnel(
        ...
        for flow_id in range(self.params.flow_count):
            ...
            # first host
            m1.__setattr__(f"netns{flow_id}", NetNamespace(f"subnet{flow_id}"))
            m1_netns = m1.__getattr__(f"netns{flow_id}")

            # generate per-netns VethPair device attributes
            m1.tmp_veth, m1.tmp_veth_peer = VethPair()
            m1.br0.port_add(m1.tmp_veth)

            # store the devices under custom names
            m1.rename_object("tmp_veth", f"veth{flow_id}")
            m1_veth = m1.__getattr__(f"veth{flow_id}")
            m1.rename_object("tmp_veth_peer", f"veth_peer{flow_id}")
            m1_veth_peer = m1.__getattr__(f"veth_peer{flow_id}")
            ...
            # second host
            m2.__setattr__(f"netns{flow_id}", NetNamespace(f"subnet{flow_id}"))
            m2_netns = m2.__getattr__(f"netns{flow_id}")

            # generate per-netns VethPair device attributes
            m2.tmp_veth, m2.tmp_veth_peer = VethPair()

            m2.br0.port_add(m2.tmp_veth)

            # store the devices under custom names
            m2.rename_object("tmp_veth", f"veth{flow_id}")
            m2_veth = m2.__getattr__(f"veth{flow_id}")
            m2.rename_object("tmp_veth_peer", f"veth_peer{flow_id}")
            m2_veth_peer = m2.__getattr__(f"veth_peer{flow_id}")
    ...
    def generate_ping_configurations(self, config):
        ...
        If we want to run ping pings in parallel (self.params.ping_parallel=True)
        we need to group all the veth_peer endpoints into single list.
    ...
    def generate_ping_endpoints(self, config):
        ...
        return [
            ...
            for first, second
            in zip(
                [self.matched.host1.__getattr__(f"netns{flow_id}").veth_peer for flow_id in range(self.params.flow_count)],
                [self.matched.host2.__getattr__(f"netns{flow_id}").veth_peer for flow_id in range(self.params.flow_count)]
            )
        ]
