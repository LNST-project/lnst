...
class XDPForwardingRecipe(ForwardingRecipe):
    ...
    def test_wide_configuration(self, config):
        ...
        job = self.matched.host2.run(
            f"xdp-forward load {self.matched.host2.eth0.name} {self.matched.host2.eth1.name}"
        )
        if not job.passed:
    ...
    def test_wide_deconfiguration(self, config):
        ...
        job = self.matched.host2.run(
            f"xdp-forward unload {self.matched.host2.eth0.name} {self.matched.host2.eth1.name}"
        )
        if not job.passed:
