...
class ForwardingMeasurement(BaseFlowMeasurement):
    ...
    def __init__(
        ...
        self._forwarder_monitor_job = None  # interface stats monitor

        self._finished_generator_job = None
        self._finished_dropper_job = None
        self._finished_fwd_monitor_job = None
    ...
    def start(self):
        ...
        self._forwarder_monitor_job.start(bg=True)
    ...
    def finish(self):
        try:
            ...
            self._forwarder_monitor_job.kill(signal.SIGINT)
            self._forwarder_monitor_job.wait()
            self._dropper_job.wait(timeout=self._dropper_job.what.runtime_estimate())
        finally:
            self._generator_job.kill()
            self._forwarder_monitor_job.kill()
            self._dropper_job.kill()

        self._finished_generator_job = self._generator_job
        self._finished_dropper_job = self._dropper_job
        self._finished_fwd_monitor_job = self._forwarder_monitor_job

        self._generator_job = None
        self._dropper_job = None
        self._forwarder_monitor_job = None

    def collect_results(self):
        results = []

        receiver_results = self._parse_dropper_results()
        forwarder_results = self._parse_fwd_monitor_results()
        ...
        for net_flow in self._net_flows:
            ...
            result = ForwardingMeasurementResults(
                measurement=self,
                measurement_success=receiver_results
                and generator_results
                and generator_results,
                flow=flow,
            ...
            result.generator_results = generator_results[
                PktgenDevice.name_template(
                    self._real_dev(flow.generator_nic), flow.generator_cpupin[0]
                )
            ]
    ...
    def _parse_dropper_results(self):
        ...
        result = (
            ParallelPerfResult()
        )  # just placeholder to match generator results hierarchy
    ...
    def _parse_generator_results(self) -> dict[str, ParallelPerfResult]:
        ...
        for nic, raw_results in self._finished_generator_job.result.items():
            results = ParallelPerfResult()  # multiple pktgen threads
    ...
    def _parse_fwd_monitor_results(self):
        result = (
            ParallelPerfResult()
        )  # just placeholder to match generator results hierarchy
        results = SequentialPerfResult()
        if not self._finished_fwd_monitor_job.passed:
            return result

        results.extend(
            self.parse_forwarder_samples(
                self._finished_fwd_monitor_job.result, "rx_packets", "packets"
            )
        )
    ...
    def _spread_results(
        self, device_results: SequentialPerfResult
    ) -> ParallelPerfResult:
        """
