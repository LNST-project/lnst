...
class ForwardingMeasurementResults(XDPBenchMeasurementResults):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # forwarding measurement doesn't measure CPU
        # but FlowMeasurementResults requires some CPU stats
        # so these are just dummy results
        self.generator_cpu_stats = self._dummy_results()
        self.receiver_cpu_stats = self._dummy_results()

    def _dummy_results(self):
        results = ParallelPerfResult()
        results.append(SequentialPerfResult())
        results[0].append(PerfInterval(0, 0, "percent", 0))
